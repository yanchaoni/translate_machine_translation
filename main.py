...
def main(args):
    ...
    input_lang, output_lang, train_pairs, train_max_length = prepareData("train", args.language, "en", args.data_path, args.max_len_ratio=args.max_len_ratio)
    _, _, dev_pairs, _ = prepareData('dev', args.language, 'en', path=args.data_path, args.max_len_ratio=0.99999)
    ...
    params = {'args.batch_size':args.batch_size, 'shuffle':False, 'collate_fn':vocab_collate_func, 'num_workers':20}
    params2 = {'args.batch_size':args.batch_size, 'shuffle':False, 'collate_fn':vocab_collate_func, 'num_workers':20}
    ...
    if args.decoder_type == "basic":
        decoder = DecoderRNN(output_lang.n_words, EMB_DIM, args.decoder_hidden_size,
                            args.decoder_layers, target_embedding, dropout_p=0.1, args.device=args.device).to(args.device)
    ...
    trainIters(encoder, decoder, train_loader, dev_loader, \
                ...
                args.epoch, args.plot_every=args.plot_every, \
                args.learning_rate=args.learning_rate, args.device=args.device, args.teacher_forcing_ratio=args.teacher_forcing_ratio, label=args.save_model_name,
                use_lr_scheduler = True, gamma_en = 0.99, gamma_de = 0.99, 
                args.beam_width=args.beam_width, args.min_len=args.min_len, args.n_best=args.n_best, args.decode_method=args.decode_method, args.save_result_path = args.save_result_path)
